...
import { typeColors } from "../../helpers/utils";
    ...
    <div className="select-container">
      <div className="buttons-container">
        {selectedPokemons.length < 2 ? (
          ...
          <button
            className={`button-start ${winner ? "disabled-button" : ""} ${
              equallity ? "disabled-button" : ""
            }`}
      ...
      <ul className="pokemons-container">
        {selectedPokemons.map((pokemon, index) => (
          <li
            className={`pokemon${index + 1} ${
              pokemonAnimations[index] ? `pokemon-animation-${index + 1}` : ""
            }`}
            key={index}
          >
            <img
            ...
            <TbPokeball
              ...
              className={selectedPokemons.length > 1 ? "icon-poke" : ""}
            />
          </li>
        ))}
        {selectedPokemons.length < 2 && (
      ...
      {pokemonVisibility && (
        <div
          className={`choice-container ${
            pokemonVisibility ? "visible" : "hidden"
          }`}
        >
          <PokemonSelection />
        </div>
